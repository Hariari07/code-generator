...
const inputFile = path.join(__dirname, "../inputs/backend.json");
const outputDir = path.join(__dirname, "../output/backend");

if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });

const { tableName, dbName, primaryKey, fields } = JSON.parse(
...
async function getRefNo() {
  ...
  try {
    ...
    console.log("Error fetching RefNo: " + error.message);
    throw error;
  }
}
...
module.exports = { getRefNo, getAll, getSingle, insert, update };
`;
...
module.exports = { fetchRefNo, fetchAll, fetchSingle, add, modify };
`;
...
async function getRefNo(req,res){
  try{
    const data = await service.fetchRefNo();
    res.json({ data });
  }catch(e){
...
async function update(req,res){
  try{
    const data = req.body;
    const result = await service.modify(data);
...
module.exports = { getRefNo, getAll, getSingle, add, update };
`;
...
// ------------------- WRITE FILES -------------------
fs.writeFileSync(path.join(outputDir, `${tableName}.model.js`), modelContent.trim());
fs.writeFileSync(path.join(outputDir, `${tableName}.service.js`), serviceContent.trim());
fs.writeFileSync(path.join(outputDir, `${tableName}.controller.js`), controllerContent.trim());
fs.writeFileSync(path.join(outputDir, `${tableName}.routes.js`), routesContent.trim());
